...
# shellcheck disable=SC2086 disable=SC2143
function handle_error() {
  local yaml_file="$1"
  local image_url="$2"
  # shellcheck disable=SC2155
  local tag="$(echo "$image_url" | cut -d ':' -f 2)"
  if [[ -n "$($SCRIPT_DIR/find_image.sh "${image_url%:*}" x86_64 2> /dev/null | jq -r '.Digest')" ]] ; then
    if [[ "$ARCH" == "x86_64" ]] ; then
  ...
  elif [[ -n $(echo $image_url | grep 'openj9') ]] && (( $(echo "$tag" | awk '{ print ($1 < 2.4)}') )) ; then
  ...
  # shellcheck disable=SC2086 disable=SC2045 disable=SC2046
