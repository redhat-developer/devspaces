...
#
# script to install an IIB as a catalog source to a logged-in cluster
# once installed, can then use that catalog source to install an operator (DevWorkspace, Dev Spaces, Web Terminal)
...
usage() {
  cat <<EOF
This script streamlines testing IIB images by configuring an OpenShift cluster
to enable it to use the specified IIB image in a catalog
...
Options:
  ...
  -n, --namespace           : namespace to install CatalogSource into. Default: openshift-operators
...
kind: CatalogSource
metadata:
  name: iib-testing-catalog
  namespace: $NAMESPACE
spec:
  sourceType: grpc
  image: ${IIB_IMAGE}
  publisher: IIB testing
