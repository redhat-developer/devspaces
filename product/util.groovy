...
def cloneRepoPoll(String URL, String REPO_PATH, String BRANCH, boolean withPolling=false, String excludeRegions='', String includeRegions='*') {
  if (URL.indexOf("pkgs.devel.redhat.com") == -1) {
    ...
    URL=URL - ~/http(s*):\/\// - ~/.*@/
    // As of 2021-08-17, new personal access token (ghp_*) URL format requires https://token@ instead of https://token:x-oauth-basic@
    def AUTH_URL_SHELL="https://" + GITHUB_TOKEN + "@" + URL
    def AUTH_URL_GROOVY="https://" + GITHUB_TOKEN + "@" + URL
