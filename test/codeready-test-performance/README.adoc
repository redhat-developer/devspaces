...
------------------
In the **working-sceletong-perf-test-with-cloning.jmx** file implemented next scenario:
* Creates virtual user(s)=amount of the treads (-Dtest.threads property)
* Each virtual user login and create a workspace
* Each virtual user waits workspace creation (RUNNING) status
* Each user run command like git clone git.url
* Waiting exec 0 status from EXEC agent (this is mean succesfull cloning)
* After cloning by user(s), the workspace is stoped  and removed
* In the end - performs clean up actions in the separated Threadgroup amd all users are removed